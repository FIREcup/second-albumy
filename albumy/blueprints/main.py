import os
from flask import render_template, Blueprint, current_app, request, send_from_directory, flash, redirect, url_for, abort
from flask_login import login_required, current_user

from ..decorators import confirm_required, permission_required
from ..extensions import db
from ..models import Photo, Comment, Tag
from ..utils import rename_image, resize_image, flash_errors
from ..forms.main import CommentForm, DescriptionForm, TagForm
...
def photo_previous(photo_id):
    photo = Photo.query.get_or_404(photo_id)
    photo_p = Photo.query.with_parent(photo.author).filter(Photo.id < photo_id).order_by(Photo.id.desc()).frist()
...
def delete_photo(photo_id):
    ...
    photo_n = Photo.query.with_parent(photo.author).filter(Photo.id > photo.id).first()
    if photo_n is None:
        photo_p = Photo.query.with_parent(photo.author).filter(Photo.id < photo.id).first()
        if photo_p is None:
...
def delete_comment(comment_id):
    ...
    return redirect(url_for('.show_photo', photo_id=comment.photo_id))  # comment is a python object
